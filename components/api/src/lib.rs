    ...
    loop {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let message = parser::parse_proto_message(buf[2], test).unwrap();

                let response_type: ProtoMessage;
                let response_content: Vec<u8>;
                match message {
                    ProtoMessage::HelloRequest(hello_request) => {
                        println!("APIVersion: {}.{} from {}", hello_request.api_version_major, hello_request.api_version_minor, hello_request.client_info);

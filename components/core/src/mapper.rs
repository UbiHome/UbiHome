            ...
            D: Deserializer<'de>,
        {
            ...
            impl<'de> Visitor<'de> for ItemsVisitor {
                    ...
                    V: SeqAccess<'de>,
                {
                    ...
                    while let Some(item) = seq.next_element::<$component_type>()? {
                        let key = "item.get_object_id();".to_string();
                        match map.entry(key) {
                            std::collections::hash_map::Entry::Occupied(entry) => {
                                return Err(serde::de::Error::custom(format!(
                                    "Duplicate entry {}",
...
#[macro_export]
macro_rules! template_mapper_kind {
    ($mapper_name:ident, $component_name:ident, $component_type:ident) => {
        fn $mapper_name<'de, D>(de: D) -> Result<Option<HashMap<String, $component_type>>, D::Error>
        where
            D: Deserializer<'de>,
                    ...
                    V: SeqAccess<'de>,
                {
                    ...
                    while let Some(item) = seq.next_element::<$component_type>()? {
                        ...
                        match map.entry(key) {
                            ...
                            std::collections::hash_map::Entry::Vacant(entry) => {
                                // match item {
                                //     $component_type::$component_name(ext) => {
                                //         debug!("Added: {}", ext.platform.as_str());
                                //     }
                                //     _ => {
                                //         debug!("Skipped: {:?}", item);
                                //     }
